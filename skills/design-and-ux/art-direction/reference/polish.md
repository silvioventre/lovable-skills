...
A scan result is defect evidence, not proof of quality. Inspect the rendered experience and real interaction path.
...
Look for the most recent critique snapshot for this target under `docs/design/critiques/`. If one exists, incorporate its relevant P0/P1 findings and name the snapshot you read. Perform an independent pass either way — a stored critique is one input, never the whole backlog.
...
Check the target against the anti-pattern families in [craft-floor.md](craft-floor.md), then run any other relevant QA commands. One scan is enough; never add a second pass over the same files. Fix real defects and document only narrow intentional exceptions. A clean scan does not replace visual judgment.
