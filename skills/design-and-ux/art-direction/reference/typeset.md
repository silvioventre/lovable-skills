...
2. **Mechanical scan:** read the target's markup and styles directly and check every type-related anti-pattern in [craft-floor.md](craft-floor.md): gradient-filled headlines, all-caps eyebrows above every heading, weight and size steps too close to read as hierarchy, arbitrary one-off values that bypass the scale, letter-spacing applied to body copy, and text set below the contrast floor. Include dynamic or computed font values, which are the ones a rule engine would miss anyway.

Keep the mechanical evidence out of the first assessment, then synthesize both passes before editing, noting what each caught alone. A clean scan is a floor, not proof of good typography.
