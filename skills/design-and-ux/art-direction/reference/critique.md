...
- Assessment A (design review) and Assessment B (mechanical evidence) are both required.
- Assessment A and B MUST stay isolated from each other. Run A to completion and write it down before you begin B; never let B's findings reach you while A is still forming.
- Assessment A must finish before mechanical findings enter the synthesis. Mechanical output is reproducible, but it still anchors judgment.
- A skipped mechanical pass is a failed critique run.
- Viewable targets require inspection of the rendered preview, not the source alone.
...
2. **Derive the target's slug** from the resolved path or URL: lowercase it, strip the protocol and any leading `src/` or `app/`, and replace every run of non-alphanumeric characters with a single hyphen. Use the same slug everywhere for this target so snapshots line up across runs. If the target cannot be resolved to a stable path, skip persistence and trend for this run, but continue the critique.
3. **Read `docs/design/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.

### Assessment Orchestration

Run Assessment A and Assessment B yourself, sequentially, and keep them genuinely isolated. Finish A and write it down in full before you begin B. Do not show findings to the user until synthesis.

The isolation is the whole mechanism, and it is easy to lose without a second process to enforce it. Two rules protect it:

- Write Assessment A out completely — every finding, with its evidence — before you look at the target again for B. An assessment you are still holding loosely is one that B will quietly reshape.
- When you synthesize, treat A as fixed evidence you are reading, not as a position you are still arguing. If B contradicts A, both go in the report; the mechanical pass does not overrule the design judgment, and the design judgment does not suppress a real defect.

Inspect the rendered preview at desktop and mobile width for any viewable target. Where an assessment needs a state the preview does not show by default (an error, an empty list, a loading pass), drive the app into that state rather than reasoning about it from source.
...
Evaluate:
- **Design specificity**: Is the composition, interaction, and visual language grounded in this product, or could an unrelated product use it unchanged? Make this judgment before running the mechanical scan.
...
Gather mechanical evidence: what is verifiable in the source and in the rendered result, independent of taste. Assessment B is mandatory and must remain isolated from Assessment A until both are complete.

**Source scan.** Read the target's markup, styles, and component code against the anti-pattern families in [craft-floor.md](craft-floor.md) and the dimensions in [audit.md](audit.md). Record every hit with its file, selector or line, and the rule it violates. A finding without a location is not a finding.

**Rendered scan.** Open the preview and check what only shows up rendered:

1. Inspect at desktop and mobile width, and at one intermediate width when the layout has a breakpoint between them.
2. Check contrast on real rendered pairs, not on token values in the abstract — computed color against computed background, including text over images and gradients.
3. Walk the keyboard path: tab order, visible focus, traps, and whether focus order matches visual order.
4. Drive each interactive surface into its non-default states — empty, loading, error, long content, overflow — and record what breaks.
5. For multi-view targets, repeat on 3-5 representative views.

Record false positives explicitly, and name any check you could not run and why.

Return: source findings with locations and rule names, rendered findings by width and state, false positives, and skipped checks with concrete reasons.

Reuse Assessment B's findings in the synthesis as recorded. Do not re-scan unless B was truncated or omitted counts, rule names, or file locations.

### Generate Combined Critique Report

Synthesize both assessments into a single report. Do NOT simply concatenate. Weave the findings together, noting where the design review and the mechanical scan agree, where the scan caught issues the design review missed, and where scan findings are false positives.
...
The report's first line MUST declare how the assessments were run, so a compromised run is never silent:
- Clean: `Method: sequential-isolated (A written and closed before B)`
- Compromised: `⚠️ A and B were not isolated (<reason>)` — use this whenever B's findings reached you before A was written out in full.
...
**Mechanical scan**: Summarize what Assessment B found, with counts and file locations. Note any issues the scan caught that the design review missed, and flag any false positives.

**Rendered inspection**: State the widths and states you actually inspected in the preview, and name any state you could not reach and why.
...
- **Suggested command**: Which command could address this (from: adapt, animate, audit, bolder, clarify, colorize, critique, delight, distill, document, harden, layout, onboard, optimize, overdrive, polish, quieter, shape, typeset)
...
#### Run Notes
Keep this compact. Include status for target slug, ignore list, assessment isolation, source scan, and rendered inspection. For failed or skipped steps, give the concrete observed reason and what you did instead.
...
Once the report above is finalized, write it to `docs/design/critique/` so the user can refer back, and so `polish` can pick up the priority issues without a copy-paste.

Skip this step if the Setup slug was null (vague or root-level target).

1. **Write the snapshot** to `docs/design/critiques/<slug>-<YYYY-MM-DD>.md`, creating the directory when it does not exist. Use the full critique report (heuristic table, design-specificity verdict, priority issues, persona red flags, minor observations, and questions), but stop before the "Ask the User" section — the questions belong in chat, not in the record.

2. **Open the file with YAML frontmatter** carrying the structured metadata, so later runs can read a trend without parsing prose:
   ...
   total_score: <n>
   max_score: <n>
   na_heuristics: "<comma-separated numbers, or empty>"
   p0_count: <n>
   p1_count: <n>
   ---
   ```

   `max_score` is the applicable maximum from the heuristic table (40 when every heuristic applied), so a later run can tell a renormalized total from a full one.

3. **Read the trend**: list the most recent snapshots for this slug in `docs/design/critiques/` and read the frontmatter of the last five, including the one you just wrote.
...
5. **Append a single line to the user-visible output**, after the report and before the questions:
   ...
   > Wrote `docs/design/critique/<filename>`.
...
**After presenting findings**, use targeted questions based on what was actually found. STOP and ask directly in chat to clarify what you cannot infer. These answers will shape the action plan.
...
- Only recommend commands from: adapt, animate, audit, bolder, clarify, colorize, critique, delight, distill, document, harden, layout, onboard, optimize, overdrive, polish, quieter, shape, typeset
