...
Run these two passes yourself, in this order, and keep them genuinely separate: finish and write down the first before you begin the second.
...
2. **Mechanical scan:** read the target's markup and styles directly and check every layout anti-pattern in [craft-floor.md](craft-floor.md): spacing values off the scale, everything centered by default, uniform gaps that flatten grouping, cards nested inside cards, containers with no max measure, horizontal overflow at mobile width, and touch targets under the minimum. Then open the preview at desktop and mobile width and confirm what the source implied. Include arbitrary spacing, overflow, and stacking behavior that only shows up rendered.

Keep the mechanical evidence out of the first assessment, then synthesize both passes before editing. A clean scan cannot prove hierarchy or rhythm.
