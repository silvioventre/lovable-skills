...
Look for an existing `PRODUCT.md` at the project root. Update it instead of creating a competing authority. In a child app inheriting root context, confirm shared versus app-specific scope before writing.
...
When the platform you just recorded is `ios`, `android`, or `adaptive`, load [ios.md](ios.md), [android.md](android.md), or both before any design work. On a project that had no PRODUCT.md, Setup could not know the platform and so never loaded them; init is the only place that learns the answer.
...
When image generation is available in this environment, ask once how new surfaces should be built, stated as the trade it is: **comp-first** (an image sets the bar before any code; bolder composition, slower, and the build must match the image) or **code-first** (build directly; the ambition is written into the direction contract and audited at the finish; leaner, faster). Record the answer in PRODUCT.md under `## Build path` as the bare value `comp` or `code`. This is a default, not a lock: the user may flip it for a single surface at any direction round. Without image generation there is no choice to record; code-first is the only path.
...
- Empty or early project: ask naturally for the surface to be built, or use `shape <surface>` when the user wants a confirmed brief without implementation. New-work will establish a visual world only when the requested work needs one.
...
If init was invoked by another request, resume it now; the native reference above is the one thing Setup could not have given you, and new-work owns later visual decisions.
