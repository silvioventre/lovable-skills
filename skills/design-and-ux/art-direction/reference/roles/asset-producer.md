This is a role you run inline, on yourself. Step fully out of the work you just finished, adopt only this file's instructions for the pass, and judge the artifact as delivered work rather than as your own intentions. Where the text below addresses a parent agent, you are both parties: produce the full output contract first, then act on it yourself.
...
6. Use whatever image generation the environment offers. If none is available, say so plainly and recommend the semantic HTML/CSS/SVG route for every item in the `produce` bucket rather than shipping a placeholder.
...
9. Save outputs non-destructively in the requested project directory, and leave the intent with the file: record the prompt used for each asset in the manifest you return, because whoever composes the page needs to know what it is looking at.
10. Compare each output against its source crop, opening every image by its workspace-relative path. Review each one against the brief before writing the final manifest, and retry each major or fatal finding once before finalizing.
